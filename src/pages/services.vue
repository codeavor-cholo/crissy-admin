<template>
    <q-page padding>
                                ...
                                <div class="q-pa-xs col-xs-12 col-sm-6 col-md-3 col-lg-3 grid-style-transition q-ma-sm" :style="props.selected ? 'transform: scale(0.95);' : ''">
                                    <q-card class="my-card" style="border: 2px solid;border-color: purple;" >
                                            <q-list>
                                                <q-item>
                                                    <q-item-section>
                                                        <q-item-label overline> {{props.row.services}} <b>{{props.row.price}}php</b></q-item-label>
                                                    </q-item-section>
        ...
        <q-page-sticky position="bottom-right" :offset="[80, 50]">
            <q-btn label="Add New Services" icon="add" color="accent" @click="addServicesDialog = true, isEdit = false" />
                <q-tooltip>
                    Add Services
                </q-tooltip>
        </q-page-sticky>
        ...
        <q-dialog v-model="addServicesDialog" persistent>
            <q-card style="min-width: 400px">
                ...
                <q-card-section>
                    <q-input class="q-ma-sm" outlined color="purple" v-model="services" label="Services"/>
                ...
                <q-card-actions align="right" class="text-primary">
                    ...
                    <q-btn v-if="!isEdit" flat color="purple" label="Add Services" v-close-popup @click="addServices"/>
                    <q-btn v-if="isEdit" flat label="Update Services" style="color: #FFC2C2" v-close-popup v-on:click="setTask"/>
                </q-card-actions>
            </q-card>
        </q-dialog>
    </q-page>
